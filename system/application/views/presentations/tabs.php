<script type="text/javascript">
	$(function() {
		$("#tabs").tabs();
	});
	</script>
	<br/>
<div id="tabs" style="width:795px;">
	<ul>
		<li><a href="#tabs-1">Data Sheets</a></li>
		<li><a href="#tabs-2">Presentations</a></li>
		<li><a href="#tabs-3">Sales Documentation</a></li>
		<li><a href="#tabs-4">Other Resource</a></li>
	</ul>
	



<div id="tabs-1">


<img src="<?=base_url()?>images/pdf_button.png"></img>
<a href="<?=base_url()?>images/files/Features_Benefits_Overview_All.pdf" target="_blank">Features and Benefits</a><br/>

<img src="<?=base_url()?>images/pdf_button.png"></img>
<a href="<?=base_url()?>images/files/access_security.pdf" target="_blank">Access and Security</a><br/>

<img src="<?=base_url()?>images/pdf_button.png"></img>
<a href="<?=base_url()?>images/files/channelpartners.pdf" target="_blank">Channel Partner</a><br/>

<img src="<?=base_url()?>images/pdf_button.png"></img>
<a href="<?=base_url()?>images/files/faqs.pdf" target="_blank">FAQs</a><br/>

<img src="<?=base_url()?>images/pdf_button.png"></img>
<a href="<?=base_url()?>images/files/Lease-DeskVersion2.pdf" target="_blank">Lease Desk Version 2</a><br/>

<img src="<?=base_url()?>images/word.png"></img>
<a href="<?=base_url()?>images/files/Lease-DeskV2.0Summary.docx" target="_blank">Lease Desk Version 2 Summary</a><br/>


</div>

<div  id="tabs-2">
<img src="<?=base_url()?>images/powerpoint.png"></img>
<a href="<?=base_url()?>images/files/LeaseDeskPPoint.ppt" target="_blank">Lease-Desk Powerpoint Presentation</a><br/>

<img src="<?=base_url()?>images/flash.png"></img>
<a href="<?=base_url()?>images/files/Lease-Desk_Flash_Presentation.exe" target="_blank">Lease-Desk Flash Presentation (.exe)</a><br/>

</div>

<div id="tabs-3">
<img src="<?=base_url()?>images/pdf_button.png"></img>
<a href="<?=base_url()?>images/files/faqs.pdf" target="_blank">FAQs</a><br/>


<img src="<?=base_url()?>images/word.png"></img>
<a href="<?=base_url()?>images/files/CoverPage.docx" target="_blank">Cover Page (DOCX format)</a><br/>
</div>
<div id="tabs-4">

<a href="https://lease-desk.webex.com/lease-desk/ldr.php?AT=pb&SP=MC&rID=30533252&rKey=60638E30D2EDD786" target="_blank">Conference Call 25th June 2010</a><br/>


<img src="<?=base_url()?>images/word.png"></img>
<a href="<?=base_url()?>images/files/WebExUserGuide.docx" target="_blank">WebEx User Guide (DOCX format)</a><br/>

<a href="http://www.proctorconsulting.co.uk/email_images/email_july_2010/email_july_2010%20copy.htm" target="_blank">Email Campaign 1</a><br/>

<a href="http://www.proctorconsulting.co.uk/email_images/email_july_2010/email_july_2010_part_2.htm" target="_blank">Email Campaign 2</a><br/>

<a href="http://www.proctorconsulting.co.uk/email_images/email_july_2010/email_july_2010_part_3.htm" target="_blank">Email Campaign 3</a><br/>

<a href="http://www.proctorconsulting.co.uk/email_images/email_sept_2010/email_sept_2010.htm" target="_blank">Email Campaign 4</a><br/>

</div>
	








</div>